...
bool path_is_remote_session_uri(const std::string& path) {
    return starts_with_ci(path, "http://") || starts_with_ci(path, "https://") ||
           starts_with_ci(path, "icy://") || starts_with_ci(path, "rtsp://") ||
           starts_with_ci(path, "rtmp://") || starts_with_ci(path, "ftp://");
}
...
PlaylistSessionEntryData PlaylistSessionController::entry_from_track(const PlaylistSessionTrack& track) {
    ...
    entry.is_stream = track.is_stream || path_is_remote_session_uri(track.audio_file_path);
    return entry;
}
