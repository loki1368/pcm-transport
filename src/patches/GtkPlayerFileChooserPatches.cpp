...
void prefer_cue_over_audio_files(std::vector<std::string>* paths) {
    ...
    for (const std::string& path : *paths) {
        ...
        try {
            const CueSheet cue_sheet = CueParser::parse_file(path, 0);
            if (!cue_sheet.audio_file_path.empty()) {
                audio_paths_from_cues.insert(media_path_key(cue_sheet.audio_file_path));
            }
            for (const std::string& audio_path : cue_sheet.audio_file_paths) {
                if (!audio_path.empty()) {
                    audio_paths_from_cues.insert(media_path_key(audio_path));
                }
            }
        } catch (...) {
        }
...
void run_open_audio_dialog(GtkPlayerWindow& window) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        if (!selected_paths.empty()) {
            ...
            std::vector<std::string> paths_to_add = selected_paths;
